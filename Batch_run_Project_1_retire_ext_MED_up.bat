...
:MODU_1
 ...
 gfortran -c .\fortran_codes\trans_JP_hs_rate_retire_ext.f90      >> result_%File_Name%_retire_MED_%YYYYMMDD%.txt 2>&1
...
gfortran -o trans_JP_hs_rate_retire_ext trans_JP_hs_rate_retire_ext.o .\mpi\mpi.o .\mpi\libmsmpi.a   >> result_%File_Name%_retire_MED_%YYYYMMDD%.txt 2>&1
...
copy .\tfp_growth_rate3_MED_up.txt  .\tfp_growth_rate.txt  >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1


"C:\Program Files\Microsoft MPI\Bin\mpiexec.exe" -n %HEIRETU_NUM% trans_JP_hs_rate_retire_ext.exe   >> result_%File_Name%_retire_MED_%YYYYMMDD%.txt 2>&1

@echo on

copy  .\file_out\trantf_JP.dat .\file_out\trantf_%File_Name%_grate%tfp_rate%_retire_MED_up.dat >> result_%File_Name%_retire_MED_%YYYYMMDD%.txt 2>&1
