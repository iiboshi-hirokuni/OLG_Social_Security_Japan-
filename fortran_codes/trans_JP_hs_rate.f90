   ...
   enddo
    ...
    !
       do t=1, tmax-1
           ...
           elseif (t < 121) then
               exog(t+1,8) = exog(t,8)*(1+1*tfp_growth_rate)  ! one times tfp growth rate                  
           else
               exog(t+1,8) = exog(t,8)    
           endif
...
!           endif
           ...
           exog(t,2) = 0.8*exp(0.2*t)/(1+0.8*exp(0.2*t))*0.19                    
       enddo   
    !
       ...
       do j=1,nj
         ...
         do t=1,tmax             
             if (l==1 .or. l==3) then  ! for male
                    if (j < 64-20) then
                        ltcare(t,h)=0.0025*ltcare(t,h)
                     elseif  (j < 69-20) then
                        ltcare(t,h)=0.0224*ltcare(t,h)
                     elseif  (j < 74-20) then
                        ltcare(t,h)=0.0422*ltcare(t,h)
                     elseif (j < 79-20) then
                        ltcare(t,h)=0.0802*ltcare(t,h)
                     elseif  (j < 84-20) then
                        ltcare(t,h)=0.1537*ltcare(t,h)
                     elseif ( j < 89-20) then
                        ltcare(t,h)=0.2816*ltcare(t,h)
                     else
                        ltcare(t,h)=0.4833*ltcare(t,h)
                     endif
              else          ! for female
                     if (j < 64-20) then
                        ltcare(t,h)=0.0020*ltcare(t,h)
                     elseif  (j < 69-20) then
                        ltcare(t,h)=0.0160*ltcare(t,h)
                     elseif  (j < 74-20) then
                        ltcare(t,h)=0.0351*ltcare(t,h)
                     elseif  (j < 79-20) then
                        ltcare(t,h)=0.0842*ltcare(t,h)
                     elseif  (j < 84-20) then
                        ltcare(t,h)=0.1958*ltcare(t,h)
                     elseif ( j < 89-20) then
                        ltcare(t,h)=0.3899*ltcare(t,h)
                     else
                        ltcare(t,h)=0.6679*ltcare(t,h)
                     endif
              endif        
         enddo
       enddo
...
!    myo      = -1
  ...
  !
    ...
    do while ((idone==0).and.(it<maxit_2))
