...
!                                    omax=na*nh*8,tbeq=1,nbeq=4
  ...
  real(8), dimension(nh)            :: eps,popi, eps_ext  ! add 2021/02/23
  ...
  real(8)                           :: crit,sum1,nrm, tfp_growth_rate, &
                             mrate_2030_1, lrate_2030_1,mrate_2030_2, lrate_2030_2  ! Add 2021/04/15
...
!  blelow we set them constant
    ...
    read(12,*) mrate_2030_1 
    read(12,*) mrate_2030_2  
    read(12,*) lrate_2030_1
    read(12,*) lrate_2030_2
   ...
   enddo
    ...
    do h=1,nh
      ...
      !
      ! Retirement extension  2021/02/23
      !      
      if (h>45 .and. h<51) then
          eps_ext(h) = 1.1*0.7 ! Male regular
      elseif (h>101+45 .and. h<101+51) then
          eps_ext(h) = 0.8*0.7 ! Female regular 
      elseif (h>2*101+45 .and. h<2*101+51) then
          eps_ext(h) = 0.7*0.7  ! Male contigent  
      elseif  (h>3*101+45 .and. h<3*101+51) then
          eps_ext(h) = 0.55*0.7  ! Feale contigent
      else            
           eps_ext(h) = eps(h)
      endif     
  ...
  ! increment of long term care cost  
    ! add 2021/08/14
     do l=1,ne
       do j=1,nj
         h  = (l-1)*nj+j
         do t=1,tmax             
             if (l==1 .or. l==3) then  ! for male
                     if (j < 64-20) then 
                        ltcare(t,h)=0.0034*ltcare(t,h) 
                     elseif  (j < 69-20) then 
                        ltcare(t,h)=0.0308*ltcare(t,h)  
                     elseif  (j < 74-20) then 
                        ltcare(t,h)=0.0581*ltcare(t,h)    
                     elseif (j < 79-20) then 
                        ltcare(t,h)=0.1112*ltcare(t,h)    
                     elseif  (j < 84-20) then 
                        ltcare(t,h)=0.2150*ltcare(t,h) 
                     elseif ( j < 89-20) then 
                        ltcare(t,h)=0.3895*ltcare(t,h)    
                     else
                        ltcare(t,h)=0.6245*ltcare(t,h)   
                     endif
              else          ! for female
                    if (j < 64-20) then 
                        ltcare(t,h)=0.0028*ltcare(t,h) 
                     elseif  (j < 69-20) then 
                        ltcare(t,h)=0.0246*ltcare(t,h)  
                     elseif  (j < 74-20) then 
                        ltcare(t,h)=0.0565*ltcare(t,h)    
                     elseif  (j < 79-20) then 
                        ltcare(t,h)=0.1394*ltcare(t,h)    
                     elseif  (j < 84-20) then 
                        ltcare(t,h)=0.3089*ltcare(t,h) 
                     elseif ( j < 89-20) then 
                        ltcare(t,h)=0.5503*ltcare(t,h)    
                     else
                        ltcare(t,h)=0.7943*ltcare(t,h)   
                     endif               
             endif
         enddo
       enddo
     enddo
    !  
   ! increment of medical cost and LTC cost 
    ! add 2021/08/14
     do l=1,ne
       do j=1,nj
         h  = (l-1)*nj+j
         do t=2,tmax
             if (t>1 .and. t < 100 ) then
                 health(t,h)=(1+0.013*0.9**(t-1))*health(t-1,h) 
                 ltcare(t,h)=(1+0.011*0.9**(t-1))*ltcare(t-1,h) 
             else
                health(t,h)=health(t-1,h)
                 ltcare(t,h)=ltcare(t-1,h)
             endif
         enddo
       enddo
     enddo
    ...
    !
    !  Rate of increase of Co pay of medical expenditure
    !
     ...
     do l=1,ne
      do j=1,nj
        ...
        do t=1,tmax
          ...
          if (t>15 .and. j > 75-20) then
               medrate(t,h)= medrate(t,h)- mrate_2030_1  
          elseif  (t>15 .and. j > 70-20) then
               medrate(t,h)= medrate(t,h)- mrate_2030_2 
          endif
   ...
   !
   !  Rate of increase of copay of long term care
   !
    ...
    do l=1,ne
      do j=1,nj
        ...
        do t=1,tmax         
          if (t>15 .and. j > 75-20) then
               ltcrate(t,h)= ltcrate(t,h)- lrate_2030_1  
          elseif  (t>15 .and. j > 65-20) then
               ltcrate(t,h)= ltcrate(t,h)- lrate_2030_2    
          endif
        enddo
      enddo
    enddo
  ...
  if (nproc>1) then
     ...
     !  Add 2021/02/23 eps_ext(h)
     call mpi_BCAST(eps_ext,nh,MPI_REAL8,0,MPI_COMM_WORLD,ierr)
  endif
...
!    myo      = -1
    if (id==0) write(*,*) 'Residuals in transition:'
      ...
      medrate, ltcare, ltcrate,prop_agent, eps_ext)
      call mpi_BARRIER(MPI_COMM_WORLD,ierr)
...
subroutine trans(x,nh,ne,nj,tmax,omax,hmax,na,eps_in,popi,pst,xit,pens,health,tbeq,nbeq,    &
...
!  include 'mpif.h'
  ...
  integer                                    :: h,i,j,k,l,s,t,i1,i2,j1,j2,   &
                                                ...
                                                retire_ext_year   !add 2021/02/23
  real(8), dimension(tmax,5)         :: x
  real(8), dimension(npar),intent(in)           :: par
  real(8), dimension(nh),intent(in)             :: eps_in,popi,eps_ext  !add 2021/02/23 
  real(8), dimension(nh)                        :: eps  !add 2021/02/23 
  ...
  ! Retirement extension   2021/02/23
  !
   eps= eps_in              !add 2021/02/23 
   retire_ext_year = 15     !add 2021/02/23    15-> 2015+15=2030
  ...
  do ic=ic1,ncohort
    ...
    ! Retirement extension   2021/02/23
    !
     if (s  < 55-retire_ext_year+1) then
         eps=eps_ext
     endif 
