...
:MODU_1
 ...
 gfortran -c .\fortran_codes\trans_JP_hs_rate.f90      >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1
...
gfortran -o trans_JP_hs_rate trans_JP_hs_rate.o .\mpi\mpi.o .\mpi\libmsmpi.a   >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1
...
copy .\tfp_growth_rate3_MED.txt  .\tfp_growth_rate.txt  >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1


"C:\Program Files\Microsoft MPI\Bin\mpiexec.exe" -n %HEIRETU_NUM% trans_JP_hs_rate.exe   >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1

@echo on

copy  .\file_out\trantf_JP.dat .\file_out\trantf_%File_Name%_grate%tfp_rate%.dat >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1

copy .\file_out\trantf_JP.nxt .\file_out\trantf_%File_Name%.nxt >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1



copy .\tfp_growth_rate6.txt  .\tfp_growth_rate.txt  >> result_%File_Name%_MED_%YYYYMMDD%.txt 2>&1
