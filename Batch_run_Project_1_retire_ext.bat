...
:MODU_1
 ...
 gfortran -c .\fortran_codes\trans_JP_hs_rate_retire_ext.f90      >> result_%File_Name%_%YYYYMMDD%.txt 2>&1
...
gfortran -o trans_JP_hs_rate_retire_ext trans_JP_hs_rate_retire_ext.o .\mpi\mpi.o .\mpi\libmsmpi.a   >> result_%File_Name%_%YYYYMMDD%.txt 2>&1
...
"C:\Program Files\Microsoft MPI\Bin\mpiexec.exe" -n %HEIRETU_NUM% trans_JP_hs_rate_retire_ext.exe   >> result_%File_Name%_retire_%YYYYMMDD%.txt 2>&1

@echo on

copy  .\file_out\trantf_JP.dat .\file_out\trantf_%File_Name%_grate%tfp_rate%_retire.dat >> result_%File_Name%_retire_%YYYYMMDD%.txt 2>&1
