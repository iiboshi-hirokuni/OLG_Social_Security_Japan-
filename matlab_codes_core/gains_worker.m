function [WG,Ufs,Ucs,Disc,logCjs,logCfs] = gains_worker(curpolt,newpolt);
...
cc(:,46:101)=0.001*ones(239,101-45);
cc(:,46+101:2*101)=0.001*ones(239,101-45);
cc(:,46+2*101:3*101)=0.001*ones(239,101-45);
cc(:,46+3*101:4*101)=0.001*ones(239,101-45);
...
for i=1:ne
  ...
  for j=2:nj;
    ...
    %%  add 2021/07/26
%        if retire_age>j+1
%             Cj(retire_age-j+1:nj-j+1,1) = 0.0001*ones(nj+1-retire_age,1);  %size(Cj)
%        else
%             Cj= 0.0001.*Cj;
%        end   
  ...
  for t=1:T-nj+1;
    Cj          = diag(c(t:t+nj-1,:));        
    %%  add 2021/07/26
%      Cj(retire_age:nj-1,1) = 0.0001*ones(nj-1-retire_age+1,1); %size(Cj)
...
cf = cf(2:end,:);
lf = lf(2:end,:);
cf(:,46:101)=0.001*ones(239,101-45);
cf(:,46+101:2*101)=0.001*ones(239,101-45);
cf(:,46+2*101:3*101)=0.001*ones(239,101-45);
cf(:,46+3*101:4*101)=0.001*ones(239,101-45);
