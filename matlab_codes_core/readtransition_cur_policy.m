...
tauc      = trantfWPHGhighertfp(i+1+2*T:i+3*T);  CUR.tauc= tauc;
...
i         = i+3*T*(ntx+ntr);
KT1       = trantfWPHGhighertfp(i+1:i+T);        CUR.KT1= KT1;
KT2       = trantfWPHGhighertfp(i+1+T:i+2*T);    CUR.KT2= KT2;
KI1       = trantfWPHGhighertfp(i+1+2*T:i+3*T);  CUR.KI1= KI1;
KI2       = trantfWPHGhighertfp(i+1+3*T:i+4*T);  CUR.KI2= KI2;
L1        = trantfWPHGhighertfp(i+1+4*T:i+5*T);  CUR.L1= L1;
L2        = trantfWPHGhighertfp(i+1+5*T:i+6*T);  CUR.L2= L2;
C         = trantfWPHGhighertfp(i+1+6*T:i+7*T); CUR.C= C;
Y         = trantfWPHGhighertfp(i+1+7*T:i+8*T);  CUR.Y= Y;
...
GDP       = trantfWPHGhighertfp(i+1+15*T:i+16*T);  CUR.GDP= GDP;
Bnew      = trantfWPHGhighertfp(i+1+16*T:i+17*T);
Ltax      = trantfWPHGhighertfp(i+1+17*T:i+18*T);  CUR.Ltax= Ltax;
Psi       = trantfWPHGhighertfp(i+1+18*T:i+19*T);
irate     = trantfWPHGhighertfp(i+1+19*T:i+20*T);  CUR.irate= irate;
w         = trantfWPHGhighertfp(i+1+20*T:i+21*T);  CUR.w= w;
...
l         = l';
at        = at';
aggC      = sum((popt.*c)')'./sum(popt')'; CUR.aggC= aggC;
aggL      = sum((popt.*l.*(ones(T,1)*eps(:)'))')'./sum(popt')'; CUR.aggL= aggL;
popul     = sum(popt');
popul     = popul';
aggA      = sum((popt.*a)')'./sum(popt')'; CUR.aggA= aggA;
...
disp('  ----------------                --------------------------------------------------          ') 
disp('  Exogenous ICURuts                 2-10    11-20    21-30    31-60    61-89   90-200  BG Path')
disp('  ----------------                ------------------------------------------------------------')
