...
tauc      = trantfWPHGhighertfp(i+1+2*T:i+3*T);  NP4.tauc= tauc;
...
i         = i+3*T*(ntx+ntr);
KT1       = trantfWPHGhighertfp(i+1:i+T);            NP4.KT1= KT1;
KT2       = trantfWPHGhighertfp(i+1+T:i+2*T);    NP4.KT2= KT2;
KI1       = trantfWPHGhighertfp(i+1+2*T:i+3*T);  NP4.KI1= KI1;
KI2       = trantfWPHGhighertfp(i+1+3*T:i+4*T);  NP4.KI2= KI2;
L1        = trantfWPHGhighertfp(i+1+4*T:i+5*T);
L2        = trantfWPHGhighertfp(i+1+5*T:i+6*T); 
C         = trantfWPHGhighertfp(i+1+6*T:i+7*T);  NP4.C= C;
Y         = trantfWPHGhighertfp(i+1+7*T:i+8*T);  NP4.Y= Y;
...
GDP       = trantfWPHGhighertfp(i+1+15*T:i+16*T);  NP4.GDP= GDP;
Bnew      = trantfWPHGhighertfp(i+1+16*T:i+17*T);
Ltax      = trantfWPHGhighertfp(i+1+17*T:i+18*T);  NP4.Ltax= Ltax;
Psi       = trantfWPHGhighertfp(i+1+18*T:i+19*T);
irate     = trantfWPHGhighertfp(i+1+19*T:i+20*T);  NP4.irate= irate;
w         = trantfWPHGhighertfp(i+1+20*T:i+21*T);  NP4.w= w;
...
l         = l';
at        = at';
aggC      = sum((popt.*c)')'./sum(popt')'; NP4.aggC= aggC;
aggL      = sum((popt.*l.*(ones(T,1)*eps(:)'))')'./sum(popt')'; NP4.aggL= aggL;
popul     = sum(popt');
popul     = popul';
aggA      = sum((popt.*a)')'./sum(popt')'; NP4.aggA= aggA;
